...
export class AuthController extends Controller {
  ...
  static async login(req, res) {
    ...
    if (!user) return invalidCredentialsFlash(req, res);

    const isPasswordMatch = await bcrypt.compare(password, user.password);

    if (!isPasswordMatch) return invalidCredentialsFlash(req, res);
    ...
    return flash({
      req,
      res,
      message: ["success", "Successfully login to your account."],
      redirectTo: "/",
    });
  }
